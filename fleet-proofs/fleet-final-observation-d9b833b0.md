...
Candidate: `d9b833b07cb9671cc5b76bda4b4a4905e8129e60`

This record binds the unchanged 2,446-key operator ledger from `fc5d7b64ca30b68c648a6ebc6cddf68ee204f39d` to the final candidate and adds the two final PayPal observations. It does not replace an impacted live surface with a generic build. The current four strict manifests were compared by exact `(domain, scenario, assertion class)` key: all 2,446 prior keys remain present with no stale key, and the only two additions are the PayPal replay and portal-cancellation replay assertions described below. The exact candidate is the already-verified `f966761eb2ad3e0026422e61a1c4094abc37543a` product tree plus release and CI orchestration changes, their static policy verifier, and an explicit electron-builder `--publish never` packaging flag; no packaged runtime payload, schema, dashboard, bot, launcher runtime, or service implementation file changed.

## Exact-final structural and production readback

- GitHub commit `d9b833b07cb9671cc5b76bda4b4a4905e8129e60` is the squash merge of PR #430. It removes `GH_TOKEN` from matrix packaging and forces electron-builder to `--publish never`, reserving draft creation and asset upload for the single controlled post-verification release job. The packaging inputs and runtime payload remain unchanged.
- GitHub PR CI run `32215473331` passed all 26 jobs on the merged source tree, including rendered browser tests, integration/live-stack lanes, four strict fleet shards, the strict aggregate, and CI Gate. The exact-candidate main run regenerates the same 46-domain, 2,448-key product manifest because the delta only removes an unintended GitHub publishing side effect.
