...
Candidate: `2edc9d0e4d4416d9b04b96e3db7c9943df3204ad`

This record binds the unchanged 2,446-key operator ledger from `fc5d7b64ca30b68c648a6ebc6cddf68ee204f39d` to the final candidate and adds the two final PayPal observations. It does not replace an impacted live surface with a generic build. The current four strict manifests were compared by exact `(domain, scenario, assertion class)` key: all 2,446 prior keys remain present with no stale key, and the only two additions are the PayPal replay and portal-cancellation replay assertions described below. The exact candidate is the already-verified `f966761eb2ad3e0026422e61a1c4094abc37543a` product tree plus release and CI orchestration changes in `.github/workflows/release.yml` and `.github/workflows/ci.yml` and their static policy verifier; no runtime, package, schema, dashboard, bot, launcher, or service implementation file changed.

## Exact-final structural and production readback

- GitHub commit `2edc9d0e4d4416d9b04b96e3db7c9943df3204ad` is the squash merge of PR #429. It removes cross-platform release-asset filename collisions, excludes debug metadata from public release checksums/provenance, and avoids an external Ubuntu mirror stall in the browser gate while retaining the real Chromium test run. The product and deployment trees are byte-identical to the verified parent.
- GitHub PR CI run `32212631398` passed all 26 jobs on the merged source tree, including rendered browser tests, integration/live-stack lanes, four strict fleet shards, the strict aggregate, and CI Gate. The exact-candidate main run regenerates the same 46-domain, 2,448-key product manifest because only CI/release orchestration and its verifier changed.
- The VPS checkout and all four production services were verified at the product parent SHA. All runtime files are unchanged by this candidate; services are healthy, 284/284 migrations are applied with no checksum drift, and Supabase, Valkey, Lavalink, Discord gateway, public HTTPS, Caddy, and Tailscale checks passed. The checkout and last-good metadata are advanced to the candidate after its green GitHub gate without restarting identical runtime images.
