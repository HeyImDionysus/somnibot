 ...
 * V3 Audit: Dashboard API route test coverage.
 */
...
describe('GET /api/reconciliation', () => {
  ...
  it('returns runs and summary when authenticated', async () => {
    ...
    expect(body.summary.last_completed).toBe('2026-01-01');
  });
});
...
describe('POST /api/reconciliation', () => {
  ...
  it('returns 409 when reconciliation is already running', async () => {
    (requireGuildOwner as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      ctx: { guildId: 'guild-123' },
    });
