 ...
 * generated by the launcher and passed via env vars. In hosted deploys,
  ...
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    ...
    if (!result.success) {
      ...
      console.error(
        ...
        'For VPS/hosted deploys, set them in the host environment.\n',
      );
      ...
      // a hosted deploy could accidentally configure those secrets and otherwise
      // hide invalid environment config.

      // In launcher local mode, CSRF is exempt, so missing CSRF_SECRET is
      // non-fatal.
      //
      // Also don't hard-exit in serverless compatibility mode. `process.exit(1)` turns
      ...
      if (!isLauncherLocalMode && process.env.VERCEL !== '1') {
        console.error('Aborting: hosted deploy requires all env vars. Fix the above and restart.\n');
        process.exit(1);
