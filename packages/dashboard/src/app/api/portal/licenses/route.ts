...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: keys, error: keysError } = await admin
      .from('license_keys')
      .select('id, key_prefix, key_suffix, status, max_devices, expires_at, created_at, products(name, type, product_license_config(rotation_policy, self_service_device_removal)), license_sessions(id, device_name, device_fingerprint, ip_address, active, first_seen_at, last_seen_at)')
