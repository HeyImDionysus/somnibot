...
export async function GET(request: NextRequest) {
  ...
  const [{ data: guild }, { data: config }] = await Promise.all([
    ...
    admin.from('guild_config').select('store_brand_source, store_brand_name, portal_brand_source, brand_primary_color, brand_accent_color, store_show_powered_by').eq('guild_id', guildId).maybeSingle(),
  ]);
