-- License owner controls that were previously represented only by hardcoded
-- values in the issuance and SDK surfaces.
--
-- Prefix is intentionally limited to uppercase ASCII so it can never change
-- the key grammar or introduce separators/control characters. Keys remain
-- hash-only at rest; `store_keys_hashed` is a locked owner-surface fact, not a
