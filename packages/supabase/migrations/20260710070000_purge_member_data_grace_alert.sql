-- =============================================================================
-- [commerce] purge_member_data — resolve stranded grace-period alerts (W2)
--
-- The GDPR /forgetme purge RPC (purge_member_data, last defined in
-- 20260621000000_v9_audit_remediation) transitions the member's entitlements
-- from 'active'/'pending'/'grace_period' to 'cancelled'. That is a TERMINAL
-- transition for a grace_period row — exactly the case the W2 grace lifecycle
-- work resolves everywhere else (EntitlementService.revoke/reconciliation, the
-- dashboard manual PUT / refund route, and the PayPal expiry/refund webhooks).
-- This RPC was the one grace-ending writer that still bypassed the resolution,
...
BEGIN
  ...
  -- W2: cancelling the entitlements above is a TERMINAL transition for any
  -- grace_period row among them — resolve the open 'entitlement_grace_period'
  -- operator alert so a churned-then-purged customer does not strand it
  -- forever. Same entitlement-scoped, unresolved-only pattern as
  -- EntitlementService.revoke/reactivate, the reconciliation sweep, the
  -- dashboard manual PUT / refund route and the PayPal webhook handlers.
  -- A no-op when no such alert is open.
  UPDATE public.alerts a
  SET resolved    = true,
      resolved_at = now(),
      updated_at  = now()
  WHERE a.guild_id = p_guild_id
    AND a.alert_type = 'entitlement_grace_period'
    AND a.resolved = false
    AND (a.metadata->>'entitlement_id') IN (
      SELECT e.id::text
