...
#
# Load VALKEY_PASSWORD from the repo .env: the local Valkey container requires
# it (docker-compose --requirepass), and a fleet run without it produces a
# NOAUTH cascade — valkey_disconnected alerts in every scenario, failing every
# happy-path owner-notification assertion with a false alarm.
ENV_FILE="$(cd "$(dirname "$0")/../.." && pwd)/.env"
if [ -f "$ENV_FILE" ]; then
  VALKEY_PASSWORD="$(grep -m1 '^VALKEY_PASSWORD=' "$ENV_FILE" | cut -d= -f2-)"
