...
export interface PlatformEventMap {
  ...
  /** Entry attempts are hot (button clicks) — audited via the batched event rail only. */
  'giveaway.entry_denied': { giveawayId: string; userId: string; reason: 'role_gate' | 'level_gate' | 'already_entered'; requiredRoleId?: string | null; requiredLevel?: number | null; userLevel?: number };
  ...
  /** One event for the whole /voice owner-control surface (lock/unlock/limit/name/permit/deny/ban). */
  'temp_channel.settings_changed': { channelId: string; actorId: string; op: 'lock' | 'unlock' | 'limit' | 'name' | 'permit' | 'deny' | 'ban'; targetUserId?: string; value?: string | number; before?: Record<string, unknown>; after?: Record<string, unknown> };
