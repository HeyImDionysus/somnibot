...
const EVENT_TO_AUDIT: Record<string, AuditMapping> = {
  ...
  'music.runtime_outage': {
    action: 'music.runtime_outage',
    category: 'music',
    targetType: 'music_runtime',
    actorType: 'system',
    targetId: (d) => d.episodeId as string,
    details: (d) => ({ revision: d.revision, source: d.source, nodeName: d.nodeName, reason: d.reason }),
    success: false,
    occurrenceId: (d) => `music-runtime:${d.episodeId as string}:outage`,
    correlationId: (d) => `music-runtime:${d.episodeId as string}`,
  },
  'music.recovery_attempted': {
    action: 'music.recovery_attempted',
    category: 'music',
    targetType: 'music_runtime',
    actorType: 'system',
    targetId: (d) => d.episodeId as string,
    details: (d) => ({ revision: d.revision, attempt: d.attempt, trigger: d.trigger }),
    occurrenceId: (d) => `music-runtime:${d.episodeId as string}:attempt:${d.attempt as number}`,
    correlationId: (d) => `music-runtime:${d.episodeId as string}`,
  },
  'music.recovery_succeeded': {
    action: 'music.recovery_succeeded',
    category: 'music',
    targetType: 'music_runtime',
    actorType: 'system',
    targetId: (d) => d.episodeId as string,
    details: (d) => ({ revision: d.revision, attempt: d.attempt, trigger: d.trigger }),
    occurrenceId: (d) => `music-runtime:${d.episodeId as string}:recovered`,
    correlationId: (d) => `music-runtime:${d.episodeId as string}`,
  },
  'music.recovery_failed': {
    action: 'music.recovery_failed',
    category: 'music',
    targetType: 'music_runtime',
    actorType: 'system',
    targetId: (d) => d.episodeId as string,
    details: (d) => ({ revision: d.revision, attempt: d.attempt, trigger: d.trigger, error: d.error, terminal: d.terminal }),
    success: false,
    occurrenceId: (d) => `music-runtime:${d.episodeId as string}:failed:${d.attempt as number}`,
    correlationId: (d) => `music-runtime:${d.episodeId as string}`,
  },
  'music.queue_preserved': {
    action: 'music.queue_preserved',
    category: 'music',
    targetType: 'music_queue',
    actorType: 'system',
    targetId: (d) => d.episodeId as string,
    details: (d) => ({ revision: d.revision, trackCount: d.trackCount, reason: d.reason }),
    occurrenceId: (d) => `music-runtime:${d.episodeId as string}:preserved`,
    correlationId: (d) => `music-runtime:${d.episodeId as string}`,
  },
  'music.manual_reconcile_required': {
    action: 'music.manual_reconcile_required',
    category: 'music',
    targetType: 'music_runtime',
    actorType: 'system',
    targetId: (d) => d.episodeId as string,
    details: (d) => ({ revision: d.revision, reason: d.reason, trackCount: d.trackCount }),
    success: false,
    occurrenceId: (d) => `music-runtime:${d.episodeId as string}:manual-reconcile`,
    correlationId: (d) => `music-runtime:${d.episodeId as string}`,
  },
  'giveaway.started': {
