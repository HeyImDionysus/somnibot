...
async function collectMemberData(
  ...
  const [
    ...
    sb.from('economy_inventory').select('*, economy_items(name, description, rarity)').eq('guild_id', guildId).eq('user_id', userId),
    ...
    sb.from('level_rewards').select('*').eq('guild_id', guildId),
    ...
    sb.from('economy_pets').select('*').eq('guild_id', guildId).eq('owner_id', userId),
    sb.from('economy_user_achievements').select('*, economy_achievement_defs(name, description)').eq('guild_id', guildId).eq('user_id', userId),
    ...
    sb.from('infractions').select('id, type, reason, moderator_id, created_at, expires_at, active').eq('guild_id', guildId).eq('user_id', userId).order('created_at', { ascending: false }),
    sb.from('tickets').select('id, ticket_number, type, status, created_at, closed_at').eq('guild_id', guildId).eq('creator_id', userId).order('created_at', { ascending: false }),
    sb.from('poll_votes').select('*, polls(title), poll_options(label)').eq('guild_id', guildId).eq('user_id', userId),
  ]);
