 ...
 * everywhere at once.
 *
 * THE CONTRACT: the 'default' preset matches the dominant live copy shipped
 * today (see __tests__/branding-voice.test.ts, which pins each template
 * against its canonical call sites). Verified BYTE-IDENTICAL at their cited
 * sites: 'unavailable' (store-command.ts:53, ticket-interactions.ts:52,
 * lottery-manager.ts:241), 'disabled' (crafting-manager.ts:116,
 * farming-manager.ts:114, lottery-manager.ts:318), 'cooldown'
 * (crafting-manager.ts:194, gathering-manager.ts:211), 'success'
 * (ticket-interactions.ts:328), 'denied' (moderation/commands.ts:165 et al,
 * purge-command.ts:67) and 'not_found' (ticket-interactions.ts:297,
 * moderation/commands.ts:330, polls-manager.ts:255) — moving THOSE sites onto
 * voice() is provably behavior-neutral for every unconfigured guild.
 *
 * EXCEPTION — 'insufficient_funds' is only the shared KERNEL of its cited
 * sites; all three diverge from it today:
 *   - pets-manager.ts:325    appends ' Check your /balance.' (and hardcodes
 *                            'coins' instead of the guild currency)
 *   - polls-manager.ts:559   prefixes '❌ Payment failed — you need …'
 *   - lottery-manager.ts:367 appends a '({count} × {price})' breakdown
 * Wave-1 migration must NOT move those three sites onto voice() as-is: either
 * keep their extra copy around the voice() kernel, or accept the
 * member-visible copy change deliberately (and say so in the PR).
 *
 * Do NOT edit a 'default' template without updating the snapshot test AND
 * accepting the member-visible copy change.
 *
...
const VOICE_TABLE: Record<BrandVoicePreset, Record<VoiceKey, string>> = {
  ...
  default: {
    ...
    // KERNEL ONLY — pets-manager.ts:325, polls-manager.ts:559 and
    // lottery-manager.ts:367 all add copy around this line today (see the
    // module doc); do not migrate them onto voice() without handling that.
