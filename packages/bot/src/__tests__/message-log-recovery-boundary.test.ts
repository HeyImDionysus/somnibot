...
describe('message-log recovery boundary', () => {
  ...
  it('does not latch on a delivered ping whose alert row failed to insert (round 33)', async () => {
    // The inverse of round 12: the Discord notice landed but the alerts-table
    // insert transiently failed. Latching then meant the outage never gained
    // a durable dashboard alert — the next failure past the backoff must
    // attempt the insert again.
    const send = vi.fn().mockRejectedValue(Object.assign(new Error('forbidden'), { status: 403 }));
    raiseOwnerAlert
      .mockResolvedValueOnce({ inserted: false, delivered: true })
    ...
    vi.useFakeTimers();
    try {
      await logMessageDelete(client as any, message(send, 'row-fail-1'));
      vi.advanceTimersByTime(61_000);
      await logMessageDelete(client as any, message(send, 'row-fail-2'));
      vi.advanceTimersByTime(61_000);
      await logMessageDelete(client as any, message(send, 'row-fail-3'));
    } finally {
      vi.useRealTimers();
    }

    // Attempt 2 re-ran the insert (row landed + delivered → latched);
    // attempt 3 was swallowed by the now-legitimate latch.
    expect(raiseOwnerAlert).toHaveBeenCalledTimes(2);
  });
