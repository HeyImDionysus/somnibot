/**
 * Tests for ../features/polls/polls-manager.js — instantiation and lifecycle.
 */
...
vi.mock('../features/polls/poll-renderer.js', () => ({
  renderPoll: vi.fn(() => ({ embed: {}, components: [] })),
  renderPrediction: vi.fn(() => ({ embed: {}, components: [] })),
}));
...
describe('PollsManager', () => {
  ...
  describe('getConfig', () => {
    it('returns null when no config', async () => {
      // PollsManager only takes supabase
    });
  });
});
