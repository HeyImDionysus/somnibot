...
- Generate the random secrets for you (`CSRF_SECRET`, `NEXTAUTH_SECRET`,
  `WEBHOOK_REPLAY_SECRET`, `LAVALINK_PASSWORD`)
...
# These four are filled in for you by the setup script — leave them alone.
# (Re-running setup never overwrites values that are already set.)
CSRF_SECRET=<generated by setup>
NEXTAUTH_SECRET=<generated by setup>
WEBHOOK_REPLAY_SECRET=<generated by setup>
LAVALINK_PASSWORD=<generated by setup>
...
> Generate `CSRF_SECRET`, `NEXTAUTH_SECRET`, and `WEBHOOK_REPLAY_SECRET` separately with `node scripts/gen-secret.mjs`; generate `LAVALINK_PASSWORD` with `node scripts/gen-secret.mjs 16`. Do not reuse your Supabase or Discord secrets. For production local or VPS callbacks, `SOMNIBOT_PUBLIC_CALLBACK_BASE_URL` and `NEXT_PUBLIC_APP_URL` should both use the stable public HTTPS dashboard base.
...
| `CSRF_SECRET` | Generate with `node scripts/gen-secret.mjs` |
| `NEXTAUTH_SECRET` | Generate with `node scripts/gen-secret.mjs` |
| `WEBHOOK_REPLAY_SECRET` | Dedicated webhook replay secret; generate with `node scripts/gen-secret.mjs` |
...
| `LAVALINK_PORT` | `2333` | — |
| `LAVALINK_PASSWORD` | Required | Generate with `node scripts/gen-secret.mjs 16`; used by local Docker Compose and the bot |
| `VALKEY_URL` | `redis://127.0.0.1:6379` | Use `redis://:<password>@valkey:6379` on VPS Docker Compose |
| `VALKEY_PASSWORD` | Optional locally; required by `docker-compose.prod.yml` | Generate with `node scripts/gen-secret.mjs 16` |
